...
# PDF transform matrices where key is the angle from x-axis in counter-clockwise
# direction
TRANSFORM_MATRIX = {
    45 : (1, 1, -1, 1),
    90 : (0, 1, -1, 0),
...
class PDFDrawOp:
    ...
    # Write PDF drawing operations corresponding to the PML object pmlOp to
    # output (util.String). pe = PDFExporter.
...
class PDFTextOp(PDFDrawOp):
    def draw(self, pmlOp: 'pml.DrawOp', pageNr: int, pe: 'PDFExporter', canvas) -> None:
        ...
        # We need to adjust y position since PDF uses baseline of text as the y
        # pos, but pml uses top of the text as y pos. The Adobe standard Courier
        # family font metrics give 157 units in 1/1000 point units as the
        # Descender value, thus giving (1000 - 157) = 843 units from baseline to
        # top of text.

        # http://partners.adobe.com/asn/tech/type/ftechnotes.jsp contains the
        # "Font Metrics for PDF Core 14 Fonts" document.
            ...
            if matrix:
                canvas.addLiteral("BT\n"\
                    ...
                    "ET\n" % (matrix[0], matrix[1], matrix[2], matrix[3],
                              # TODO: Doing this with addLiteral, non-latin
                              #  characters won't work in watermarks. There must
                              #  be a better way to do this with reportlab, that
                              #  we do it this way is for historical reasons and
                              #  because no one took the time to change it.
        ...
        if pmlOp.flags & pml.UNDERLINED:
            ...
            # All standard PDF fonts have the underline line 100 units below
            # baseline with a thickness of 50
        ...
        if pmlOp.toc:
            # We need a unique key to link the bookmark in toc
            # TODO: generate a more speaking one
            bookmarkKey = uuid.uuid4().hex
...
class PDFQuarterCircleOp(PDFDrawOp):
    def draw(self, pmlOp: 'pml.DrawOp', pageNr: int, pe: 'PDFExporter', canvas) -> None:
        ...
        # The traditional constant is: 0.552284749 however, as described here:
        # http://spencermortensen.com/articles/bezier-circle/,
        # this has a maximum radial drift of 0.027253%.
        # The constant calculated by Spencer Mortensen has a max. drift of
        # 0.019608% which is 28% better.
...
class PDFExporter:
    ...
    def generate(self) -> bytes:
        ...
        if doc.defPage != -1:
            # this should make the PDF reader open the PDF at the desired page
            # canvas.addLiteral("/OpenAction [%d 0 R /XYZ null null 0]\n" % (self.pageObjs[0].nr + doc.defPage * 2))
            # TODO: This doesn't seem to be easily doable with reportlab.
            #  /OpenAction is considered a security threat by some (as it allows
            #  executing JavaScript), so I think it's unlikely they'll add
            #  support. Also, this feature didn't work with many PDF viewers
            #  anyway; I tested Evince, Okular and pdf.js in Firefox, and they
            #  all didn't support it. So maybe, we should remove this feature
            #  entirely?
            pass
    ...
    def genStream(self, s, isFontStream = False) -> str:
        ...
        # Embedded TrueType font program streams for some reason need a Length1
        # entry that records the uncompressed length of the stream
        if isFontStream:
            lenStr = "/Length1 %d\n" % len(s)
        else:
            lenStr = ""
    ...
    # Get font name to use for given flags. Also registers the font in
    # reportlabs if it does not yet exist.
