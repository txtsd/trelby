...
# format for expressing a script's complete contents in a neutral way that's
# easy to render to almost anything, e.g. PDF, Postscript, Windows GDI, etc.
#

# A PML document is a collection of pages plus possibly some metadata. Each page
# is a collection of simple drawing commands, executed sequentially in the order
# given, assuming "complete overdraw" semantics on the output device,
# i.e. whatever is drawn completely covers things it is painted on top of.
...
class Document:
    ...
    def __init__(self, w: float, h: float):
        ...
        # User-specified fonts, if any. key = 2 lowest bits of TextOp.flags,
        # value = pml.PDFFontInfo
        ...
        # When running testcases, misc.version does not exist, so store a dummy
        # value in that case, correct value otherwise.
        self.version: str = getattr(misc, "version", "dummy_version")

        # A random string to embed in the PDF; only used by watermarked PDFs.
...
class TOCItem:
    def __init__(self, text: str, op: 'TextOp'):
        ...
        # Pointer to the TextOp that this item links to (used to get the correct
...
class PDFFontInfo:
    def __init__(self, name: str, fontFileName: Optional[str]):
        ...
        # If empty, use the default PDF font.
        self.name: str = name

        # The font program (in practise, the contents of the .ttf file for the
        # font), or None, in which case the font is not embedded.
...
# Draw text string 'text', at position (x, y) mm from the upper left corner of
# the page. Font used is 'size' points, and Courier / Times / Helvetica as
# indicated by the flags, possibly being bold / italic / underlined. Angle is
# None, or an integer from 0 to 360 that gives the slant of the text
...
# Draw consecutive lines. 'points' is a list of (x, y) pairs (minimum 2 pairs)
# and 'width' is the line width, with 0 being the thinnest possible line.
# If 'isClosed' is True, the last point on the list is connected to the first one.
class LineOp(DrawOp):
    pdfOp = pdf.PDFLineOp()
...
# Draw a rectangle, possibly filled, with specified lineWidth (which can be -1
# if fillType is FILL). (x, y) is position of upper left corner.
class RectOp(DrawOp):
    pdfOp = pdf.PDFRectOp()
...
# Draw a quarter circle centered at (x, y) with given radius and line width. By
# default, it will be the upper left quadrant of a circle, but using the
# flip[XY] parameters you can choose other quadrants.
...
# Arbitrary PDF commands. Should not have whitespace in the beginning or the
# end. Should be used only for non-critical things like tweaking line join
# styles etc., because non-PDF renderers will ignore these.
class PDFOp(DrawOp):
...
# Create a PML document containing text (possibly linewrapped) divided into
# pages automatically.
class TextFormatter:
    def __init__(self, width: float, height: float, margin: float, fontSize: int):
        ...
        # Number of characters that fit on a single line
    ...
    # Wrap text into lines that fit on the page, using Courier and default font
    # size and style, and add the lines. 'indent' is the text to prefix lines
    # other than the first one with.
