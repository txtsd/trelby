...
# Extremely rough estimate for the base memory usage of a single undo object,
# WITHOUT counting the actual textual differences stored inside it. So this
# figure accounts for the Python object overhead, member variable overhead,
# memory allocation overhead, etc.
#
...
# Convert a list of Screenplay.Line objects into an unspecified, but compact,
# form of storage. storage2lines will convert this back to the original form.
#
...
def lines2storage(lines):
    ...
    # Instead of having an arbitrary cutoff figure ("compress if < X bytes"),
    # always compress, but only use the compressed version if it's shorter than
    # the non-compressed one.
...
# Abstract base class for storing undo history. Concrete subclasses implement
# undo/redo for specific actions taken on a screenplay.
class Base:
    ...
    # Rough estimate of how much memory is used by this undo object. Can be
    # overridden by subclasses that need something different.
    def memoryUsed(self):
        return (BASE_MEMORY_USAGE + memoryUsed(self.linesBefore) +
                memoryUsed(self.linesAfter))

    # Default implementation for undo. Can be overridden by subclasses that need
    # something different.
    def undo(self, sp):
    ...
    # Default implementation for redo. Can be overridden by subclasses that need
    # something different.
    def redo(self, sp):
...
# Stores a full copy of the screenplay before/after the action. Used by actions
# that modify the screenplay globally.
#
# We store the line data as compressed text, not as a list of Line objects,
# because it takes much less memory to do so. Figures from a 32-bit machine (a
# 64-bit machine wastes even more space storing Line objects) from speedTest for
# a 120-page screenplay (Casablanca):
#
...
class SinglePara(Base):
    ...
    def setAfter(self, sp):
        # If all we did was modify a single paragraph, the index of its starting
        # line can not have changed, because that would mean one of the
        # paragraphs above us had changed as well, which is a logical
        # impossibility. So we can find the dimensions of the modified paragraph
        # by starting at the first line.
...
class ManyElems(Base):
    # Line is any line belonging to the first modified element. There is no
    # requirement for the cursor to be in this paragraph.
    # nrOfElemsStart is how many elements there are before the edit
    # operation and nrOfElemsEnd is how many there are after.
    # So an edit operation splitting an element would pass in (1, 2) while an
    # edit operation combining two elements would pass in (2, 1).
...
# Stores a single block of changed lines by diffing before/after states of a
# screenplay
...
# Our own implementation of difflib.SequenceMatcher, since the actual one is too
# slow for our custom needs.
#
# l1, l2 = lists to diff. List elements must have __ne__ defined.
#
# Return a, b, x, y such that l1[a:b] could be replaced with l2[x:y] to convert
# l1 into l2.
